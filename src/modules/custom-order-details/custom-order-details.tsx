...
import OrderProgressTracker from './components/get-order-progress-tracker';
...
export interface IOrderDetailsViewProps extends ICustomOrderDetailsProps<ICustomOrderDetailsData> {
    ...
    orderProgressTracker?: React.ReactNode;
}
...
class OrderDetails extends React.PureComponent<ICustomOrderDetailsProps<ICustomOrderDetailsData>> {
    ...
    @observable private orderHistory: string | null = null;
    ...
    @action
    public async fetchOrderDetails(): Promise<void> {
        const { request } = this.props.context;
        const { salesId = '' } = request.query || {};

        const cRetailURL = this.props.context.request.apiSettings.baseUrl;
        const cRetailOUN = this.props.context.request.apiSettings.oun || '';
        ...
        try {
            const response = await fetch(cKORGetOrderStatusRequestUrl, {
                ...
                body: JSON.stringify({ OrderId: salesId })
            });

            if (response.ok) {
                const data = await response.json();
                const status: string = data.value;

                if (status.toLowerCase() === 'order not found') {
                    this.orderHistory = null;
                } else {
                    this.orderHistory = status;
                }
            } else {
                this.orderHistory = null;
            }
    ...
    @computed
    public get _orderDetailsProgress(): string | null {
